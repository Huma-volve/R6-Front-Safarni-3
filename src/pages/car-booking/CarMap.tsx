  ...
  function LocationSelector() {
    useMapEvents({
      click(e) {
        ...
        const d = turf.distance(from, to, { units: "kilometers" as const });
        setDistance(d);
  ...
  return (
    <>
      ...
      >
        ...
        {userLocation && <Polyline positions={[fixedLocation, userLocation]} color="blue" />}
    ...
    className='w-50 h-24 object-cover rounded-xl'/>
       ...
       <div className='pb-2'>
        ...
        <div className="flex justify-between items-center pt-2">
            <div className="p-3">
                {car && <h5 className='text-center font-semibold'>${hourlyPrice.toFixed(2)}</h5>}
                <p className="text-gray-500 text-xs text-end">per hour</p>
            </div>
            <div>
                {car &&  <h5 className=' text-center font-semibold'>${price.toFixed(2)}</h5>}
                <p className="text-gray-500 text-xs text-end">per day</p>
            </div>
        </div>
        </div>
        <button className="px-6 py-2 w-full rounded-xl bg-blue-700 border-blue-700 border-2 text-white hover:bg-white hover:text-blue-700 transition-all duration-200  cursor-pointer">Confirm</button>
      </div>
      </div>
