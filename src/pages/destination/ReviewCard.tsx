...
const ReviewCard = ({ data }: { data: Review }) => {
  ...
  return (
    <div className="border-[#D1D3DB] border-[1px] rounded-lg p-2 text-2xl">
      <div className="font-medium flex justify-between items-center gap-2">
        ...
        <p className="text-[16px]">{monthsSince(data.created_at)} months ago</p>
      </div>
      <Rating value={data.rating}>
        {Array.from({ length: 5 }).map((_, index) => (
          <RatingButton key={index} className="text-yellow-400 h-5 w-5" />
        ))}
      </Rating>
      <p className="text-[22px]">{data.review}</p>
    </div>
  );
};
